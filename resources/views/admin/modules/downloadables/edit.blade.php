    ...
    <div class="row">
        <div class="card" style="width: 100%">
            ...
            <div class="card-body">
                ...
                {!! Form::model($downloadables, ['method' => 'POST','route' => ['admin.downloadables.update', $downloadables->id], 'files'=>true]) !!}
                {!! Form::hidden('id') !!}
                <div class="row">
                    <div class="col-sm-12 col-md-12 col-lg-8">
                        <div class="col-sm-12">
                            <div class="form-row">
                                ...
                                <button type="button" onclick="$('#attachement').next().find('.ff_fileupload_actions button.ff_fileupload_start_upload').click(); return false;" class='btn btn-secondary'>Upload all downloadables</button>
                            </div>
                        </div>
                    </div>
                </div>
...
$(document).ready(function () {
    $('#attachement').FancyFileUpload({
        url: '{{ route("admin.downloadables.update", $downloadables->id) }}',
        params: {
            '_token': $('meta[name="csrf-token"]').attr('content')
        },
        maxdownloadablesize: 2000000, //2MB
        retries : 1
    });
});
</script>
